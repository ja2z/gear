...
const ReservationInfo = () => {
  ...
  useDesktopHeader({ title: 'Reservation Details' });
              ...
              <ul className="space-y-0.5">
                {group.items.map(item => (
                  <li key={item.itemId} className="text-sm text-gray-700 flex items-baseline gap-2">
                    <span className="font-medium text-scout-orange shrink-0">{item.itemId}</span>
                    {item.description && (
  ...
  const newEventModal = showNewEventModal && (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4">
      ...
      />
      <div className="modal-dialog-panel-enter relative z-[101] max-h-[min(90dvh,40rem)] w-full max-w-md overflow-y-auto rounded-2xl bg-white px-6 pt-6 pb-[max(2rem,env(safe-area-inset-bottom,0px))] sm:pb-10 shadow-2xl">
